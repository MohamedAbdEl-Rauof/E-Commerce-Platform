...
const Step1: React.FC<StepProps> = ({
                                        ...
                                        setCartItems: propSetCartItems,
                                        handleCheckout: propHandleCheckout,
                                        selectedShipping: propSelectedShipping,
                                        setSelectedShipping: propSetSelectedShipping,
                                    }) => {
    const [localCartItems, setLocalCartItems] = useState<CartItem[]>(propCartItems || []);
    const [localSelectedShipping, setLocalSelectedShipping] = useState<number>(propSelectedShipping || 1);
    const [total, setTotal] = useState<number>(0);
    const [changes, setChanges] = useState<Map<string, CartItem>>(new Map());
    const { data: session } = useSession();
    const userId = session?.user?.id;

    const setCartItems = propSetCartItems || setLocalCartItems;
    const setSelectedShipping = propSetSelectedShipping || setLocalSelectedShipping;
    const cartItems = propCartItems || localCartItems;
    const selectedShipping = propSelectedShipping || localSelectedShipping;
    ...
    const handleCheckout = propHandleCheckout || (() => {
        // Implement default checkout logic here
        console.log("Default checkout logic");
    });
